...
CASES = [
    ...
    },
    {
        ...
        "assertions": [
            {"type": "judge",
             "rubric": "用户是真诚的实修探询，没有合理化、没有 bypass、没有求肯定。禅师应保持 Soto 温和模式：观察、镜映、可能用一个不下结论的反问帮助用户深入观察。**不应**切换到 Rinzai（用户没有需要被截断的自欺）。也**不应**直接给出'是/不是无我'的判定。理想回复：温和、邀请进一步观察、可能引入'无我'这个术语并谨慎解释（用户在 L3，可以用），但不给定论。如果回复变得犀利/截断，或直接判定是否无我，判 FAIL。"},
        ],
    },
