"""
Vocal Coach prompt — automated behavioral test harness.

Runs scripted conversations against the coach prompt and checks responses
against assertions (substring / regex / LLM-as-judge).
...
COACH_PROMPT = (ROOT / "coach-prompt.md").read_text(encoding="utf-8")
...
def coach_call(messages: list[dict]) -> tuple[str, dict]:
    """Single coach turn. Returns (assistant_text, usage_dict)."""
...
def judge_call(rubric: str, conversation: list[dict], target_idx: int) -> dict:
    """LLM-as-judge. Returns {'passed': bool, 'reason': str}."""
    transcript = []
    ...
    judge_prompt = f"""You are a strict, terse test evaluator for a vocal-coach prompt's behavior.
...
def run_case(case: dict) -> CaseResult:
    ...
    try:
        for turn in case["turns"]:
            ...
            # First user turn carries the journal as injected context
...
def render_report(results: list[CaseResult]) -> str:
    ...
    lines = []
    lines.append(f"# Vocal Coach Prompt — Test Report")
    lines.append("")
    lines.append(f"- Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"- Coach model: `{MODEL}`")
    lines.append(f"- Judge model: `{JUDGE_MODEL}`")
    lines.append("")
    lines.append(f"## Summary")
    lines.append("")
    lines.append(f"**{passed} / {total} passed** ({100 * passed // total if total else 0}%)")
    lines.append("")
    ...
    lines.append(f"- Cache read: {cache_read:,} (≈0.1× cost)")
    lines.append(f"- Cache write: {cache_write:,} (≈1.25× cost)")
    lines.append("")
    ...
    for r in results:
        ...
        if r.assertions:
            ...
            for a in r.assertions:
                e = "✅" if a.passed else "❌"
                rationale = f" — _{a.rationale}_" if a.rationale else ""
                lines.append(f"- {e} `{a.type}`{rationale}")
