"""
Zen Master prompt — automated behavioral test harness.
...
def judge_call(rubric: str, conversation: list[dict], target_idx: int) -> dict:
    ...
    judge_prompt = f"""You are a strict, terse test evaluator for a zen-master prompt's behavior.
...
def render_report(results: list[CaseResult]) -> str:
    ...
    lines = [
        f"# Zen Master Prompt — Test Report",
        "",
